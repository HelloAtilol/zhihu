...
# 相当于手动启动浏览器，然后交给selenium接管
os.popen('chrome.exe --remote-debugging-port=9222 --user-data-dir="E:\Temp\chrome"')


class ZhihuConnection:
    ...
    def GetCookies(self):
        ...
        #     login_text = browser.find_element_by_class_name("SignFlow-tabs")
        #     login_text = login_text.find_elements_by_class_name("SignFlow-tab")
        #     # 切换到密码登录
        #     login_text[1].click()
        #     browser.find_element_by_css_selector(".SignFlow-accountInput.Input-wrapper input").send_keys("15172470205")
        #     time.sleep(random.randint(1, 5))
        #     browser.find_element_by_name("password").send_keys("vawo113133")
        #     # 检查是否有验证码。如果有，输入验证码之后在控制台回车
        #
        #     submit = browser.find_element_by_class_name("SignFlow-submitButton")
        #
        #     input("暂停》》》查看结果")
        #     submit.click()
...
zhihu = ZhihuConnection('https://www.zhihu.com/')
zhihu.Crawl()
